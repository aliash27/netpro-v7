...
import {
  calcDebt, buildPaidMap, fmt, avatarColor,
  validatePhone, getToday, getCurMo
} from '../utils'
...
export default function Subscribers() {
  ...
  async function load() {
    setLoading(true)
    const [{ data, error: e1 }, { data: paysData, error: e2 }] = await Promise.all([
  ...
  function openAdd() {
    ...
    setForm({
      ...
      monthly_fee: '', last_paid_month: getCurMo(),
  ...
  async function save() {
    if (!form.name.trim() || !form.phone.trim() || !form.start_date || !form.monthly_fee) {
      toast('يرجى ملء الحقول المطلوبة *', 'e'); return
    }
    if (!validatePhone(form.phone)) {
      toast('رقم الهاتف غير صحيح — يجب أن يبدأ بـ 07', 'e'); return
    }
    if (parseFloat(form.monthly_fee) <= 0) {
    ...
    if (editId) {
      ...
      if (error) { toast('خطأ في التعديل: ' + error.message, 'e'); setSaving(false); return }
      toast('تم تعديل البيانات ✅', 's')
    } else {
      const { error } = await supabase.from('subscribers')
        .insert({ ...payload, company_id: company.id })
      if (error) { toast('خطأ في الإضافة: ' + error.message, 'e'); setSaving(false); return }
  ...
  return (
    <div className="page">
      ...
      {loading ? <SkeletonList count={6} /> : list.length === 0 ? (
          ...
          <div key={sub.id} className="sub-row"
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 5, flexShrink: 0 }}>
              ...
              {sub.subscription_end && new Date(sub.subscription_end) <= new Date(Date.now() + 7 * 86400000) && (
                <span style={{
                  fontSize: 10, background: 'rgba(225,29,72,.1)', color: '#e11d48',
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: 17, fontWeight: 800, color: 'var(--ink)', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 10 }}>
              📥 استيراد من CSV
              <button onClick={() => setShowImport(false)} style={{ marginRight: 'auto', width: 32, height: 32, borderRadius: '50%', background: 'var(--bg2)', border: 'none', cursor: 'pointer', color: 'var(--ink3)', fontSize: 15 }}>✕</button>
            </div>
            <div style={{ background: 'rgba(26,63,219,.06)', borderRadius: 12, padding: '11px 14px', marginBottom: 14, fontSize: 13, color: 'var(--ink2)', lineHeight: 1.8 }}>
            ...
            <textarea style={{
              ...
              resize: 'vertical', outline: 'none', direction: 'ltr', boxSizing: 'border-box', marginBottom: 10
            }}
              placeholder={'الاسم,الهاتف,الرسم الشهري\nأحمد محمد,07701234567,35000'}
              value={csvText}
              onChange={e => setCsvText(e.target.value)} />
            <div style={{ marginBottom: 14 }}>
              <label style={{ fontSize: 13, fontWeight: 700, color: 'var(--ink2)', display: 'block', marginBottom: 6 }}>أو ارفع ملف CSV:</label>
            ...
            <button onClick={importFromCSV} disabled={importing || !csvText.trim()} style={{
              ...
              cursor: csvText.trim() ? 'pointer' : 'not-allowed', marginBottom: 9, fontFamily: 'inherit'
            }}>
              {importing ? '⏳ جاري الاستيراد...' : '📥 استيراد المشتركين'}
            </button>
            <button onClick={() => { setShowImport(false); setCsvText('') }} style={{
              width: '100%', padding: 12, borderRadius: 12, border: '1px solid var(--bdr)',
              background: 'transparent', color: 'var(--ink3)', fontWeight: 700, fontSize: 14, cursor: 'pointer', fontFamily: 'inherit'
            }}>إلغاء</button>
          </div>
        </div>
      )}
      ...
      <style>{`
        ...
        .modal-sheet { width:100%;max-width:560px;max-height:92vh;overflow-y:auto;
          ...
          padding:10px 20px 32px;animation:slideUp .38s ease;border-top:1px solid var(--bdr); }
        .modal-pill { width:38px;height:4px;background:var(--bdr);border-radius:4px;margin:8px auto 18px; }
        ...
        .sec-count { font-size:11px;font-weight:700;color:var(--ink3);
          background:var(--bg2);border:1px solid var(--bdr);padding:3px 10px;border-radius:20px; }
